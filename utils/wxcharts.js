...
function getXAxisPoints(categories, opts, config) {
    ...
    var spacingValid = opts.width - 2*config.padding;
    var dataCount = opts.enableScroll ? Math.min(5, categories.length) : categories.length;
    var eachSpacing=0
    if(dataCount-1==0){
       eachSpacing = 0;
    }else{
       eachSpacing = spacingValid / (dataCount - 1);
    }
    ...
    categories.forEach(function (item, index) {
       if(startX+index*eachSpacing-spacingValid>0){
         xAxisPoints.push(spacingValid+config.padding-5)
       }else{
...
function drawPointShape2(item, context) {
  context.beginPath();
  context.setStrokeStyle("#ffffff");
  context.setLineWidth(1);
  context.setFillStyle("#2a9ef1");
  if (item !== null) {
 ...
 * desc=分割线
 */
function drawToolTipSplitLine(offsetX,offsetY, opts, config, context) {
...
function drawLineDataPoints(series, opts, config, context) {
    ...
    if (opts.tooltip && opts.tooltip.textList && opts.tooltip.textList.length && process === 1) {
      drawToolTipSplitLine(opts.tooltip.offset.x, opts.tooltip.offset.y, opts, config, context);
    }
    context.restore();
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
          ...
          if (index == categories.length - 1) {
            let offset = eachSpacing / 2 - measureText(item) / 2;
            context.fillText(item, xAxisPoints[index] - measureText(item) , startY + config.fontSize + 5);
          }
        });
        context.closePath();
        context.stroke();
      ...
      context.setFillStyle(opts.xAxis.fontColor || '#737373');
            categories.forEach(function (item, index) {
              /**
               * tag=bigman
               * desc=绘制x轴起始点
              ...
              if(index==0){
               ...
               offset = eachSpacing / 2 - measureText(item) / 2;
                }else{
                   offset =0;
                }
            ...
            //绘制x轴终点
              ...
              if (index == categories.length - 1){
                if (categories.length!=1){
                  let offset = eachSpacing / 2 - measureText(item) / 2 ;
                  if(item==""){
                    item = originalCat[originalCat.length-1]
                  }
                  context.fillText(item, xAxisPoints[index]  - measureText(item)+offset  , startY + config.fontSize + 5);
                }
...
function drawYAxisGrid(opts, config, context) {
    ...
    points.forEach(function (item, index) {
      ...
      if (index == 0) {
        ...
        context.lineTo(startX, points[points.length - 1])
      }
      //绘制X轴线
...
function drawYAxis(series, opts, config, context) {
    ...
    rangesFormat.forEach(function (item, index) {
         ...
         */
        if(index==0)
        context.fillText(item, config.padding + config.yAxisTitleWidth+2, pos + config.fontSize / 2);
        if (index == rangesFormat.length - 1)
          context.fillText(item, config.padding + config.yAxisTitleWidth+2 , pos );
    });
    context.closePath();
    context.stroke();
  /**
...
Charts.prototype.changeLineColor = function (color) {
  this.config.colors
  [0]=color
