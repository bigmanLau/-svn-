...
function drawYAxisTitle(title, opts, config, context) {
    ...
    context.beginPath();
    context.setFontSize(config.fontSize);
  context.setFillStyle(opts.yAxis.titleFontColor || '#999999');
...
function drawColumnDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        points.forEach(function (item, index) {
          context.beginPath();
          context.setFillStyle(opts.colors[index]);
            if (item !== null) {
...
function drawYAxisGridForColumn (opts, config, context) {
    var spacingValid = opts.height - 2 * config.padding - config.xAxisHeight - config.legendHeight;
    var eachSpacing = Math.floor(spacingValid / config.yAxisSplit);
    var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
    var startX = config.padding + yAxisTotalWidth;
    ...
    context.setLineWidth(1);
    points.forEach(function (item, index) {
      drawDashLine(context, startX, item, endX, item)
      context.moveTo(startX, item);
      context.lineTo(endX, item);
    });
    context.closePath();
    ...
    legendList.forEach(function (itemList, listIndex) {
      ...
      // var startX = opts.width - width;
        ...
        itemList.forEach(function (item) {
            ...
            context.beginPath();
          context.setFillStyle(opts.extra.legendTextColor || '#999999');
            context.fillText(item.name, startX, startY);
            context.closePath();
            context.stroke();
    ...
    switch (type) {
        ...
        case 'column':
            this.animationInstance = new Animation({
                ...
                duration: duration,
                onProcess: function onProcess(process) {
                  drawYAxisGridForColumn(opts, config, context);
